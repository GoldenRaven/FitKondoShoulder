...
coes=open('coes','r')
coe7=np.loadtxt('negative_matlab_fit_coefficient_25.dat')
...
title_=r"down, $n=0.5, \rm{steps}=5000, iter=25, (c+c')/2$"
plt.title(title_)
plt.plot(discard[:,0],discard[:,3],'kp',ms=3,label='discarded c3')
plt.plot(pick[:,0],pick[:,3],'rp-',ms=3,label='picked c3')
plt.plot(pick[:,0],-1*pick[:,2],'bp-',ms=3,label='picked -1*c2')
plt.plot(delta[:,0],delta[:,1],'b--',ms=3,label=r'equation')
plt.grid(True)
plt.legend()
plt.twinx()
plt.plot(coe7[:,0],coe7[:,6],'gp-',ms=3,label='Sum of squares due to error')
plt.legend()
plt.savefig('down.eps', dpi=None, facecolor='w', edgecolor='w', orientation='portrait', format='pdf', bbox_inches='tight')
# os._exit(0)
...
for line3 in coes:
    ...
    plt.title(title_)
    plt.plot(coe[:,2],'rp-',ms=3,label=word4[1]+', c2')
    plt.plot(coe[:,3],'bp-',ms=3,label=word4[1]+', c3')
    plt.legend()
    i=i+1
...
for line3 in coes:
    ...
    ax = plt.subplot(nrows,ncols,i+1)
    plt.title(title_)
    plt.plot(coe[:,7],'rp-',ms=3,label=word4[1]+', lower bound')
    plt.plot(coe[:,8],'bp-',ms=3,label=word4[1]+', upper bound')
    plt.legend()
    i=i+1
...
# plt.savefig('negative_fit.eps', dpi=None, facecolor='w', edgecolor='w', orientation='portrait', format='pdf', bbox_inches='tight')
