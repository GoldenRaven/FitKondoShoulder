...
iter_max=25
n=0.5
# head -n 1 ../dos-uniform_positive/dos_*
for ((iter_num=0;iter_num<=$iter_max;iter_num++))
do
    echo $iter_num > iter_num
    if [ $iter_num -eq 0 ]
    then
        rm -f positive_matlab_fit_coefficient*.dat dos_Vg0*fit* coe_h*.dat *.eps
        i=1
        for file in ../dos-uniform_positive/dos_Vg0_h0.0*
        do
            # continue
            cp $file dos_Vg_h
            ...
            echo $h "  " $c1 " " $c2 "  " $c3 "  " $c4 "  " $c5 "  "$rmse"  "$lower"   "$upper"    "$bound1"  "$bound2>> positive_matlab_fit_coefficient_$iter_num.dat
        done
        # echo hi
    elif [ $iter_num -eq 1 ]
    then
        ...
        do
            ...
            bound1=`sed -n "${i}p" positive_matlab_fit_coefficient_${iter_tmp}.dat|awk '{print $10}'`
            bound2=`sed -n "${i}p" positive_matlab_fit_coefficient_${iter_tmp}.dat|awk '{print $11}'`
            echo $bound1 $bound2 > bounds
            ((i++))
            matlab -nodesktop -nosplash -r "createFit" > tmp
            cp fit.dat ${file}_fit_$iter_num
        ...
        do
            ...
            y2=`sed -n "${i}p" init_bound|awk '{print $4}'`
            echo $init_bound1 $init_bound2 $y1 $y2> init_bounds
            bound1=`sed -n "${i}p" positive_matlab_fit_coefficient_${iter_old}.dat|awk '{print $10}'`
            bound2=`sed -n "${i}p" positive_matlab_fit_coefficient_${iter_old}.dat|awk '{print $11}'`
            bound3=`sed -n "${i}p" positive_matlab_fit_coefficient_${iter_tmp}.dat|awk '{print $10}'`
            bound4=`sed -n "${i}p" positive_matlab_fit_coefficient_${iter_tmp}.dat|awk '{print $11}'`
...
for file in positive_matlab_fit_coefficient_*.dat
do
    sort -k1n,1 $file > tmp
    mv tmp $file
done
# rm -f positive_self-consistent
# mkdir positive_self-consistent
# mv ../dos-uniform_positive/dos_Vg0_h*_fit positive_self-consistent
bash same_h.bash
python positive_fit.py
rm -f tmp fit.dat # ../dos-uniform_positive/*_fit
date >> date
